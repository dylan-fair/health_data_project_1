...
# age group as adult, older adult, child
...
age_comparison.plot(kind='barh', figsize=(10, 7), width=0.8)
...
# age group(s) in each study
bc_age_count2 = breast_cancer['Age'].value_counts()
bc_age_percent2 = bc_age_count2 / bc_age_count2.sum() * 100

covid_age_count2 = covid['Age'].value_counts()
covid_age_percent2 = covid_age_count2 / covid_age_count2.sum() * 100

age_comparison2 = pd.DataFrame({
    'Breast Cancer': bc_age_percent2,
    'COVID': covid_age_percent2})

age_comparison2.plot(kind='barh', figsize=(10, 7), width=0.8)
plt.title('Comparison of Age Studied in Breast Cancer and COVID')
plt.xlabel('Age Group Percentage')
plt.ylabel('')
